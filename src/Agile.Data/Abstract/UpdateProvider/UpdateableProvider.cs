...
namespace Agile.Data.Abstract
{
    ...
    {
        #region Property
        public AgileProvider Context { get; internal set; }
        ...
        public KeyValuePair<string, List<AgileParameter>> ToSql()
        {
            ...
            RestoreMapping();
            return new KeyValuePair<string, List<AgileParameter>>(sql, UpdateBuilder.Parameters);
        }
        ...
        public IUpdateable<T> Where(string fieldName, string conditionalType, object fieldValue)
        {
            ...
            this.UpdateBuilder.Parameters.Add(new AgileParameter(parameterName, fieldValue));
            return this;
        }
        ...
        private void PreToSql()
        {
            ...
            if (this.IsSingle)
            {
                foreach (var item in this.UpdateBuilder.DbColumnInfoList)
                {
                    if (this.UpdateBuilder.Parameters == null) this.UpdateBuilder.Parameters = new List<AgileParameter>();
                    if (this.UpdateBuilder.SetValues.Any(it => this.SqlBuilder.GetNoTranslationColumnName(it.Key) == item.PropertyName))
                    {
                        continue;
                    }
                    this.UpdateBuilder.Parameters.Add(new AgileParameter(this.SqlBuilder.SqlParameterKeyWord + item.DbColumnName, item.Value, item.PropertyType));
                }
            }

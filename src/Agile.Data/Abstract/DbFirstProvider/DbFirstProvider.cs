...
namespace Agile.Data.Abstract
{
    ...
    {
        public virtual IAgileClient Context { get; set; }
        ...
        internal string GetClassString(DbTableInfo tableInfo, ref string className)
        {
            ...
            classText = classText.Replace(DbFirstTemplate.KeyAgileTable, IsAttribute ? string.Format(DbFirstTemplate.ValueAgileTable, tableInfo.Name) : null);
            if (columns.HasValue())
            {
        ...
        internal string GetClassString(List<DbColumnInfo> columns, ref string className)
        {
            ...
            classText = classText.Replace(DbFirstTemplate.KeyAgileTable, IsAttribute ? string.Format(DbFirstTemplate.ValueAgileTable, className) : null);
            if (columns.HasValue())
            {
                foreach (var item in columns)
                {
        ...
        {
            string agileColumnText = DbFirstTemplate.ValueAgileCoulmn;
            var propertyName = GetPropertyName(item);
            var isMappingColumn = propertyName != item.DbColumnName;
            var hasAgileColumn = item.IsPrimarykey == true || item.IsIdentity == true || isMappingColumn;
            if (hasAgileColumn && this.IsAttribute)
            {
                ...
                agileColumnText = string.Format(agileColumnText, string.Join(",", joinList));
            }
            else
            {
                agileColumnText = null;
            }
            string typeString = GetPropertyTypeName(item);
            PropertyText = PropertyText.Replace(DbFirstTemplate.KeyAgileColumn, agileColumnText);

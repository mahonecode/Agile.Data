...
namespace Agile.Data.Abstract
{
    public class InsertableProvider<T> : IInsertable<T> where T : class, new()
    {
        ...
        public virtual KeyValuePair<string, List<AgileParameter>> ToSql()
        {
            ...
            RestoreMapping();
            return new KeyValuePair<string, List<AgileParameter>>(sql, InsertBuilder.Parameters);
        }
        ...
        protected virtual void PreToSql()
        {
            ...
            if (this.IsSingle)
            {
                ...
                {
                    if (this.InsertBuilder.Parameters == null) this.InsertBuilder.Parameters = new List<AgileParameter>();
                    var paramters = new AgileParameter(this.SqlBuilder.SqlParameterKeyWord + item.DbColumnName, item.Value, item.PropertyType);
        ...
        {
            List<AgileParameter> parameters = new List<AgileParameter>();

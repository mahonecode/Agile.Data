...
namespace Agile.Data.Abstract
{
    public abstract partial class SqlBuilderProvider : SqlBuilderAccessory, ISqlBuilder
    {
        ...
        public virtual void RepairReplicationParameters(ref string appendSql, AgileParameter[] parameters, int addIndex)
        {
            UtilMethods.RepairReplicationParameters(ref appendSql, parameters, addIndex);
        }
        public KeyValuePair<string, AgileParameter[]> ConditionalModelToSql(List<IConditionalModel> models, int beginIndex = 0)
        {
            if (models.IsNullOrEmpty()) return new KeyValuePair<string, AgileParameter[]>();
                ...
                if (model is ConditionalModel)
                {
                    ...
                    switch (item.ConditionalType)
                    {
                        case ConditionalType.Equal:
                            ...
                            parameters.Add(new AgileParameter(parameterName, GetFieldValue(item)));
                            break;
                        case ConditionalType.Like:
                            builder.AppendFormat(temp, type, item.FieldName.ToSqlFilter(), "LIKE", parameterName);
                            parameters.Add(new AgileParameter(parameterName, "%" + item.FieldValue + "%"));
                            break;
            ...
            return new KeyValuePair<string, AgileParameter[]>(builder.ToString(), parameters.ToArray());
        }

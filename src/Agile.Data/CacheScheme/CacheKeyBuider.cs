...
namespace Agile.Data.CacheScheme
{
    internal class CacheKeyBuider
    {
        ...
        private static void AddTables(IAgileClient context, QueryBuilder queryBuilder, CacheKey result)
        {
            result.Tables = new List<string>();
            result.Tables.Add(context.EntityProvider.GetTableName(queryBuilder.EntityName));
            if (queryBuilder.EasyJoinInfos.HasValue())
            {
                foreach (var item in queryBuilder.EasyJoinInfos)
                {
                    result.Tables.Add(context.EntityProvider.GetTableName(item.Value));
                }
            }

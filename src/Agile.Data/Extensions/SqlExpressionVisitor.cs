...
namespace Agile.Data.Extensions
{
    public class SqlExpressionVisitor
    {
        ...
        protected void VisitNewArray(NewArrayExpression na, SqlVistorFlag flag)
        {
            ...
            {
                var e = original[i];
                if (e.NodeType == ExpressionType.NewArrayInit || e.NodeType == ExpressionType.NewArrayBounds)
                {
        ...
        protected virtual void VisitMember(MemberExpression exp, SqlVistorFlag flag)
        {
            if (exp.Expression != null && exp.Expression.NodeType == ExpressionType.Parameter)
            {
                ...
                {
                    if (exp.Member.DeclaringType.GetTypeInfo().IsGenericType && exp.Member.DeclaringType.GetGenericTypeDefinition() == typeof(Nullable<>))
                    {
                        ...
                        var doesNotEqualNull = Expression.MakeBinary(ExpressionType.NotEqual, exp.Expression, Expression.Constant(null));
    ...
    public class SqlVistorFlag
    {
        ...
        public SqlVistorFlag(SqlVistorType vistorType)
        {
            ...
            DbProvider = DapperExtensions.Configuration.ConnConfig.DbType;
        }
        ...
        private DatabaseType DbProvider { get; set; } = DatabaseType.MySql;
